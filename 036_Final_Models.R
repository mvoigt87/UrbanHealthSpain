...
## 5.0 Model results in table
    ...
    ## -----------------------    
    ## Model test for reviewer
    ## -----------------------
    library(flexsurv)
    
    GOMP_UI_A <- flexsurvreg(Surv(time = age.entry,
                                   time2 = age.exit,
                                   event = event) ~ UI.N , data = INMO.SC.2,
                             dist = "gompertz")
    ...
    GOMP_UI_B <- flexsurvreg(Surv(time = age.entry,
                                        time2 = age.exit,
                                        event = event) ~ UI.N + IP_LIMPIEZA + IP_CONTAM + IP_RUIDOS + IP_DELINC + PCT_UNEMPL +
                                     PCT_SOLTEROS + sexo + dep + ecivil +  fnac + estudios4 + tenen + vehic , data = INMO.SC.2,
                              dist = "gompertz")
